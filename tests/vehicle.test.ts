...
describe("destor::vehicle", () => {
  ...
  it("mint vehicle account", async () => {
    ...
    expect(vehicleAccount.nftAsset.equals(anchor.web3.PublicKey.default)).to.eq(
      true
    );
    expect(
      vehicleAccount.manufacturer.equals(organization.organizationPda)
    ).to.eq(true);
    expect(vehicleAccount.owner.equals(anchor.web3.PublicKey.default)).to.eq(
      true
    ); // @todo when NFT exist, dont forget
  ...
  it("assign initial owner", async () => {
    ...
    const vehicleAccount = await program.account.vehicle.fetch(
      vehicle.vehiclePda
    );

    expect(vehicleAccount.owner.equals(owner.publicKey)).to.eq(true);
    expect(vehicleAccount.ownerCount).to.eq(1);
    expect(Buffer.from(vehicleAccount.vinHash).equals(vehicle.vinHash)).to.eq(
      true
    );
    expect(
      vehicleAccount.manufacturer.equals(organization.organizationPda)
  ...
  it("success vehicle transfer", async () => {
    ...
    await program.methods
      .transferVehicle(Array.from(vehicle.vinHash), newOwner.publicKey)
      .accountsPartial({
