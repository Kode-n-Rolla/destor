...
describe("destor::organization", () => {
  ...
  it("register an organization", async () => {
    ...
    // Assertion
    expect(organizationAccount.authority.equals(authority.publicKey)).to.eq(
      true
    );
    expect(
      organizationAccount.pendingAuthority.equals(anchor.web3.PublicKey.default)
    ).to.eq(true);
    expect(
      Buffer.from(organizationAccount.organizationId).equals(organizationId)
    ).to.eq(true);
  ...
  it("set valid threshold for organization", async () => {
    ...
    const organizationBeforeChange = await program.account.organization.fetch(
      organization.organizationPda
    );
    ...
    const organizationAfterChange = await program.account.organization.fetch(
      organization.organizationPda
    );
  ...
  it("set invalid threshold for organization", async () => {
    ...
    const organizationAfterFail = await program.account.organization.fetch(
      organization.organizationPda
    );
  ...
  it("change authority", async () => {
    ...
    const organizationAfterRequestAuthorityTransfer =
      await program.account.organization.fetch(organization.organizationPda);
    expect(
      organizationAfterRequestAuthorityTransfer.pendingAuthority.equals(
    ...
    const organizationAfterAcceptAuthorityTransfer =
      await program.account.organization.fetch(organization.organizationPda);

    expect(
      organizationAfterAcceptAuthorityTransfer.authority.equals(
        newAuthority.publicKey
      )
    ).to.eq(true);
    expect(
      organizationAfterAcceptAuthorityTransfer.pendingAuthority.equals(
        anchor.web3.PublicKey.default
      )
    ).to.eq(true);
    expect(
      organizationAfterAcceptAuthorityTransfer.authority.equals(
        organization.authority.publicKey
      )
    ).to.eq(false);
  });
});
