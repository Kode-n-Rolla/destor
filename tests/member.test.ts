...
describe("destor::member", () => {
  ...
  it("add member for organization", async () => {
    ...
    expect(
      memberAccount.organization.equals(organization.organizationPda)
    ).to.eq(true);
  ...
  it("remove member from organization", async () => {
    ...
    expect(
      memberAccountAfterRemove.organization.equals(organization.organizationPda)
    ).to.eq(true);
    expect(memberAccountAfterRemove.wallet.equals(newMember.publicKey)).to.eq(
      true
    );
  });
});
